...
export async function contactsCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown contacts subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi contacts --help` to see list, get, stats, create, update, delete, import, export"]);
    }
}
/** Contacts default to email identifiers; bare digits mean contact_id. */
function identifierTypeFor(identifier, override) {
    if (override)
        return override;
    return /^\d+$/.test(identifier) ? "contact_id" : "email_id";
}
async function contactsList(args, ctx) {
    ...
    const listIds = splitCsv(values["lists"]);
    const ids = splitCsv(values["ids"]);
...
async function contactsGet(args, ctx) {
    ...
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    const payload = await ctx.client.request("GET", `/contacts/${encodeURIComponent(identifier)}`, { query: { identifierType: identifierTypeFor(identifier, identifierType) } });
    if (!payload) {
...
async function contactsStats(args, ctx) {
    ...
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    const payload = await ctx.client.request("GET", `/contacts/${encodeURIComponent(identifier)}/campaignStats`, { query: { identifierType: identifierTypeFor(identifier, identifierType) } });
    const stats = asRecordArray(payload?.["statistics"]);
    const out = {
        contact: identifier,
        campaigns: stats.map((row) => compact({
...
async function contactsCreate(args, ctx) {
    ...
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    const body = { email, updateEnabled: updateExisting };
    if (attributes)
        body["attributes"] = attributes;
    if (listIds.length > 0)
        body["listIds"] = listIds;
    requireConfirm(values, commandPath, `create contact ${email}${listIds.length > 0 ? ` in lists ${listIds.join(",")}` : ""}`);
    ...
    return renderResult({
        created: compact({
            ...
            note: updateExisting || id === undefined
...
async function contactsUpdate(args, ctx) {
    ...
    if (!attributes && listIds.length === 0 && unlinkListIds.length === 0 && !emailBlacklist && !smsBlacklist) {
        throw new AxiError("contacts update needs at least one of --attr, --lists, --unlink-lists, --email-blacklist, --sms-blacklist", "VALIDATION_ERROR", [`Run \`${commandPath} --help\` for usage`]);
    }
    const body = {};
    if (attributes)
        body["attributes"] = attributes;
    ...
    requireConfirm(values, commandPath, `update contact ${identifier} (${type}) with ${JSON.stringify(body)}`);
...
async function contactsDelete(args, ctx) {
...
async function contactsImport(args, ctx) {
    ...
    requireApiKey(ctx, commandPath);
    if (!body) {
        throw new AxiError("contacts import requires --body <json> (the import request: fileUrl, fileBody, or jsonBody plus listIds)", "VALIDATION_ERROR", ['Example: --body \'{"jsonBody":[{"email":"a@b.com"}],"listIds":[4]}\'']);
    }
...
async function contactsExport(args, ctx) {
    ...
    requireApiKey(ctx, commandPath);
    if (!body) {
        throw new AxiError("contacts export requires --body <json> (optional export filters; {} exports everyone)", "VALIDATION_ERROR", ['Example: --body \'{"exportAttributes":["EMAIL","FIRSTNAME"]}\'']);
    }
