...
import { forbidExtraPositionals, oneOf, optionalInt, parseIntCsv, parseFlags, parseJsonFlag, requirePositional, requireString, requireConfirm, splitCsv, } from "../lib/args.js";
...
export async function campaignsCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown campaigns subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi campaigns --help` to see list, get, create, update, send, test, status, report, export, delete"]);
    }
}
async function campaignsList(args, ctx) {
...
function statsSummary(stats) {
    ...
    return (`${sent} sent, ${delivered} delivered (${pct(delivered, sent)}), ` +
        `${opened} opened (${pct(opened, delivered)}), ${clicked} clicked, ${bounced} bounced`);
}
async function campaignsGet(args, ctx) {
...
async function campaignsCreate(args, ctx) {
    ...
    if (!name || !subject || !senderName || !senderEmail) {
        throw new AxiError("campaigns create requires --name, --subject, --sender-name, --sender-email", "VALIDATION_ERROR", [`Run \`${commandPath} --help\` for usage`]);
    }
    ...
    if (sources !== 1) {
        throw new AxiError("campaigns create needs exactly one content source: --html, --html-url, or --template", "VALIDATION_ERROR");
    }
    if (listIds.length === 0) {
    ...
    requireConfirm(values, commandPath, `create campaign "${name}" to lists ${listIds.join(",")} (${listIds.length} list${listIds.length > 1 ? "s" : ""})`);
...
async function campaignsUpdate(args, ctx) {
    ...
    requireApiKey(ctx, commandPath);
    if (!body) {
        throw new AxiError('campaigns update requires --body <json> (the patch object)', "VALIDATION_ERROR", ['Example: --body \'{"subject":"New subject"}\'']);
    }
    ...
    return renderResult({ updated: { id, fields: Object.keys(body) }, help: ["Run `brevo-axi campaigns get <id>` to verify"] }, json);
}
async function campaignsSend(args, ctx) {
    ...
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    requireConfirm(values, commandPath, `send campaign ${id} IMMEDIATELY to its entire audience (consumes email credits)`);
...
async function campaignsTest(args, ctx) {
    ...
    return renderResult({ tested: { campaign: id, to }, help: ["Run `brevo-axi campaigns get <id>` for campaign details"] }, json);
}
async function campaignsStatus(args, ctx) {
    ...
    requireApiKey(ctx, commandPath);
    if (!status) {
        throw new AxiError(`campaigns status requires --status (${STATUS_TARGETS.join("|")})`, "VALIDATION_ERROR");
    }
...
async function campaignsReport(args, ctx) {
    ...
    const language = oneOf(values, "language", ["en", "fr", "es", "de", "it", "pt"]) ?? "en";
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    if (to.length === 0) {
    ...
    const payloadBody = { email: { to } };
    if (body)
        payloadBody["email"] = { to, body };
    payloadBody["language"] = language;
    await ctx.client.request("POST", `/emailCampaigns/${encodeURIComponent(id)}/sendReport`, {
        body: payloadBody,
    });
    return renderResult({ report_sent: { campaign: id, to }, help: ["Check the recipients' inbox for the report"] }, json);
}
async function campaignsExport(args, ctx) {
    ...
    const payload = await ctx.client.request("POST", `/emailCampaigns/${encodeURIComponent(id)}/exportRecipients`, { body: { recipientsType } });
    return renderResult({
        export: compact({
            campaign: id,
...
async function campaignsDelete(args, ctx) {
    ...
    return renderResult({ deleted: { campaign: id }, help: ["Run `brevo-axi campaigns list` to confirm"] }, json);
}
