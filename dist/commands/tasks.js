...
import { forbidExtraPositionals, oneOf, optionalInt, parseFlags, requirePositional, requireString, requireConfirm, splitCsv, } from "../lib/args.js";
...
const TYPES_FLAGS = { json: { type: "boolean" } };
const CREATE_FLAGS = {
    name: { type: "string" },
    type: { type: "string" },
    date: { type: "string" },
...
export async function tasksCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown tasks subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi tasks --help` to see list, types, get, create, update, delete"]);
    }
}
...
async function tasksList(args, ctx) {
    const commandPath = "brevo-axi tasks list";
    const { values } = parseFlags(args, commandPath, LIST_FLAGS);
    const status = oneOf(values, "status", ["done", "not-done"]) ?? requireString(values, "status", commandPath);
...
async function tasksTypes(args, ctx) {
    ...
    if (rows.length === 0)
        out["result"] = "0 task types defined";
    out["help"] = ["Use the id in `tasks create --type <id>`"];
    return renderResult(out, json);
}
async function tasksGet(args, ctx) {
...
async function tasksCreate(args, ctx) {
    ...
    const assignee = requireString(values, "assignee", commandPath);
    const contacts = splitCsv(values["contacts"]);
    const deals = splitCsv(values["deals"]);
    const json = values["json"] === true;
    requireApiKey(ctx, commandPath);
    if (!name || !type || !date) {
        throw new AxiError("tasks create requires --name, --type <typeId>, --date <YYYY-MM-DD>", "VALIDATION_ERROR", ["Run `brevo-axi tasks types` to list type ids"]);
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
        throw new AxiError(`--date must be YYYY-MM-DD, got: ${date}`, "VALIDATION_ERROR");
    }
    const body = { name, taskTypeId: type, date };
    if (notes)
        body["notes"] = notes;
    if (assignee)
        body["assignToId"] = assignee;
    if (contacts.length > 0)
        body["contactsIds"] = contacts.map(Number).filter((n) => Number.isInteger(n));
    if (deals.length > 0)
        body["dealsIds"] = deals;
...
async function tasksUpdate(args, ctx) {
    ...
    return renderResult({ updated: compact({ id, fields: Object.keys(body) }), help: [`Run \`brevo-axi tasks get ${id}\` to verify`] }, json);
}
async function tasksDelete(args, ctx) {
    ...
    return renderResult({ deleted: { task: id }, help: ["Run `brevo-axi tasks list` to confirm"] }, json);
}
