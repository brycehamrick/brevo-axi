...
import { forbidExtraPositionals, oneOf, parseFlags, requirePositional, requireString, requireConfirm, } from "../lib/args.js";
...
const ATTRIBUTE_TYPES = ["text", "date", "float", "boolean", "category", "multiple-choice"];
export async function attributesCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown attributes subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi attributes --help` to see list, create, update, delete"]);
    }
}
async function attributesList(args, ctx) {
    ...
    const out = {
        count: rows.length,
        attributes: rows.map((row) => compact({
...
async function attributesCreate(args, ctx) {
    ...
    if (!type) {
        throw new AxiError(`attributes create requires --type (${ATTRIBUTE_TYPES.join("|")})`, "VALIDATION_ERROR");
    }
...
async function attributesUpdate(args, ctx) {
    ...
    return renderResult({ updated: compact({ name, value, options: enumeration }), help: ["Run `brevo-axi attributes list` to verify"] }, json);
}
async function attributesDelete(args, ctx) {
    ...
    return renderResult({ deleted: { name }, help: ["Run `brevo-axi attributes list` to confirm"] }, json);
}
