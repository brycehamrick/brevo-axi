...
export async function processesCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown processes subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi processes --help` to see list, get"]);
    }
}
async function processesList(args, ctx) {
    ...
    const done = rows.filter((row) => asString(row["status"]) === "done" || asString(row["status"]) === "completed").length;
    const out = {
        count: rows.length,
        total,
        breakdown: rows.length > 0 ? `${done} done, ${rows.length - done} running` : undefined,
        processes: rows.map((row) => compact({
            id: row["id"],
...
async function processesGet(args, ctx) {
