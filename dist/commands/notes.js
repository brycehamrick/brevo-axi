...
import { forbidExtraPositionals, optionalInt, parseFlags, requirePositional, requireString, requireConfirm, } from "../lib/args.js";
...
export async function notesCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown notes subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi notes --help` to see list, get, create, update, delete"]);
    }
}
async function notesList(args, ctx) {
...
async function notesGet(args, ctx) {
...
async function notesCreate(args, ctx) {
    ...
    if (!contact && !deal && !company) {
        throw new AxiError("notes create requires at least one link: --contact, --deal, or --company", "VALIDATION_ERROR");
    }
    const requestBody = { text: body };
    if (contact)
        requestBody["contactId"] = Number(contact);
    if (deal)
        requestBody["dealId"] = deal;
    if (company)
        requestBody["companyId"] = company;
...
async function notesUpdate(args, ctx) {
    ...
    return renderResult({ updated: { id }, help: [`Run \`brevo-axi notes get ${id}\` to verify`] }, json);
}
async function notesDelete(args, ctx) {
    ...
    return renderResult({ deleted: { note: id }, help: ["Run `brevo-axi notes list` to confirm"] }, json);
}
