...
import { oneOf, optionalInt, parseFlags, requireString, requireConfirm, } from "../lib/args.js";
...
export async function smsCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown sms subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi sms --help` to see send, stats, events"]);
    }
}
async function smsSend(args, ctx) {
    ...
    if (digits.length < 6 || digits.length > 15) {
        throw new AxiError(`--to must be a mobile number with country code (6-15 digits), got: ${to}`, "VALIDATION_ERROR");
    }
    let params;
    if (param) {
        try {
    ...
    const body = { recipient: to, content: text, type: "transactional" };
    if (sender)
        body["sender"] = sender;
    if (tag)
        body["tag"] = tag;
    if (params)
        body["params"] = params;
    requireConfirm(values, commandPath, `send SMS to ${to} (${text.length} chars \u2248 ${Math.max(1, Math.ceil(text.length / 160))} credit segment(s))`);
...
async function smsStats(args, ctx) {
    ...
    if (daily) {
        ...
        if (rows.length === 0)
            out["result"] = "0 daily SMS reports in the window";
...
async function smsEvents(args, ctx) {
