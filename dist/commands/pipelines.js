...
import { forbidExtraPositionals, parseFlags, requirePositional, } from "../lib/args.js";
...
export async function pipelinesCommand(args, ctx) {
    const sub = args[0];
    ...
    if (sub !== "list" && sub !== "get") {
        ...
        throw new AxiError(`unknown pipelines subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi pipelines --help` to see list, get"]);
    }
    ...
    if (sub === "get") {
        ...
        const match = pipelines.find((row) => asString(row["pipeline_name"]) === name || asString(row["pipeline"]) === name);
        if (!match) {
        ...
        return renderResult({
            ...
            stages: stages.map((stage) => compact({
