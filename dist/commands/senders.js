...
export async function sendersCommand(args, ctx) {
    const sub = args[0];
    ...
    switch (sub) {
        ...
        default:
            throw new AxiError(`unknown senders subcommand: ${sub}`, "VALIDATION_ERROR", ["Run `brevo-axi senders --help` to see list, create, update, validate, delete"]);
    }
}
export async function sendersList(args, ctx) {
...
async function sendersCreate(args, ctx) {
...
async function sendersUpdate(args, ctx) {
    ...
    return renderResult({ updated: compact({ id, name, email }), help: ["Run `brevo-axi senders list` to verify"] }, json);
}
async function sendersValidate(args, ctx) {
    ...
    return renderResult({ validated: { sender: id }, help: ["Run `brevo-axi senders list` to confirm it is active"] }, json);
}
async function sendersDelete(args, ctx) {
    ...
    return renderResult({ deleted: { sender: id }, help: ["Run `brevo-axi senders list` to confirm"] }, json);
}
