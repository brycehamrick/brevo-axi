...
import { redact } from "../lib/env.js";
const DEFAULT_TIMEOUT_MS = 60_000;
export function createBrevoClient(config, deps = {}) {
    ...
    async function request(method, path, opts = {}) {
        ...
        try {
            ...
            clearTimeout(timer);
            if (isAbort(error)) {
                throw new AxiError(`Brevo API request timed out after ${timeoutMs}ms (${method} ${path})`, "TIMEOUT", ["Retry, or narrow the request with filters"]);
            }
            throw new AxiError(`Network error reaching Brevo at ${config.apiUrl}: ${redact(String(error), config.apiKey)}`, "NETWORK_ERROR", ["Check your connection and BREVO_API_URL"]);
        }
        clearTimeout(timer);
        ...
        try {
            ...
            throw new AxiError(`Brevo API returned an unparseable response (${method} ${path}): ${bodyText.slice(0, 200)}`, "API_ERROR");
        }
    }
...
function httpError(status, bodyText, method, path, config) {
    ...
    if (status === 401 || status === 403) {
        return new AxiError(`Brevo rejected authentication (HTTP ${status}) for ${where}: ${message}`, "AUTH_REQUIRED", [
    ...
    if (status === 402) {
        return new AxiError(`Brevo account has insufficient credits (HTTP 402) for ${where}: ${message}`, "PAYMENT_REQUIRED", ["Add credits or upgrade the plan in Brevo billing"]);
    }
    if (status === 404) {
    ...
    if (status === 429) {
        return new AxiError(`Brevo rate limit hit (HTTP 429) for ${where}`, "RATE_LIMITED", ["Wait for the rate-limit window to reset, then retry"]);
    }
    return new AxiError(`Brevo API HTTP ${status} for ${where}${message ? `: ${message}` : ""}`, "API_ERROR");
}
