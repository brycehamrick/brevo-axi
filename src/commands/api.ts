...
export async function apiCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (!/^[a-zA-Z0-9/_{}.-]+\/?$/.test(path) || path.includes("..")) {
    throw new AxiError(
      `path must be relative to the v3 base (letters, digits, /, -, _, .), got: ${path}`,
      "VALIDATION_ERROR",
      ["Example: contacts/lists/4"],
    );
  }
  ...
  if (values["query"] !== undefined) {
    query = {};
    for (const pair of splitCsv(values["query"] as string)) {
  ...
  if (method !== "get") {
    requireConfirm(
      values,
      commandPath,
      `${method.toUpperCase()} /${cleanPath}${body ? ` with ${JSON.stringify(body).slice(0, 160)}` : ""}`,
    );
  }
