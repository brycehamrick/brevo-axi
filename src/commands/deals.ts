...
export async function dealsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown deals subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi deals --help` to see list, get, create, update, delete"],
      );
  }
}

export function dealRow(row: Record<string, unknown>): Record<string, unknown> {
  ...
  return compact({
    ...
    company: Array.isArray(row["linkedCompaniesIds"])
      ? (row["linkedCompaniesIds"] as unknown[]).map(String).join(",")
      : undefined,
...
async function dealsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function dealsGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function dealsCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const attributes: Record<string, unknown> = { deal_name: name, ...(stage ? { deal_stage: stage } : {}), ...attr };
  const body: Record<string, unknown> = { name, attributes };
  if (company) body["linkedCompaniesIds"] = [company];
...
async function dealsUpdate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (!name && !attr && !stage && !company && contacts.length === 0) {
    throw new AxiError(
      "deals update needs at least one of --name, --stage, --attr, --company, --contacts",
      "VALIDATION_ERROR",
    );
  }

  const attributes: Record<string, unknown> = {};
  if (name) attributes["deal_name"] = name;
  if (stage) attributes["deal_stage"] = stage;
  ...
  return renderResult(
    { updated: compact({ id, fields: Object.keys(body) }), help: [`Run \`brevo-axi deals get ${id}\` to verify`] },
    json,
  );
}

async function dealsDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { deleted: { deal: id }, help: ["Run `brevo-axi deals list` to confirm"] },
    json,
  );
}
