...
const CONTACTS_FLAGS: Record<string, FlagDefinition> = {
  limit: { type: "string" },
  offset: { type: "string" },
...
const MEMBERSHIP_FLAGS: Record<string, FlagDefinition> = {
  ids: { type: "string" },
  emails: { type: "string" },
  confirm: { type: "boolean" },
  json: { type: "boolean" },
...
export async function listsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown lists subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi lists --help` to see list, get, contacts, create, update, delete, add, remove"],
      );
  }
}

async function listsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function listsGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function listsContacts(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const payload = await ctx.client.request(
    "GET",
    `/contacts/lists/${encodeURIComponent(id)}/contacts`,
    { query: { limit, offset, modifiedSince } },
  );
...
async function listsCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function listsUpdate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function listsDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  requireConfirm(
    values,
    commandPath,
    `delete list ${id} (contacts survive, but their list membership is lost)`,
  );
...
async function listsMembership(
  action: "add" | "remove",
  args: string[],
  ctx: CommandContext,
  ...
  const emails = (values["emails"] as string | undefined)?.split(",").map((e) => e.trim()).filter(Boolean) ?? [];
  const json = values["json"] === true;
  requireApiKey(ctx, commandPath);
  ...
  requireConfirm(
    values,
    commandPath,
    `${action} ${ids.length > 0 ? `ids ${ids.join(",")}` : emails.length} ${ids.length > 0 ? "contacts" : "contacts by email"} ${action === "add" ? "to" : "from"} list ${id}`,
  );

  const payload = await ctx.client.request(
    "POST",
    `/contacts/lists/${encodeURIComponent(id)}/contacts/${action}`,
    { body },
  );
  return renderResult(
    {
