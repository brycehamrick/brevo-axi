...
export async function templatesCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown templates subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi templates --help` to see list, get, create, update, send-test, delete"],
      );
  }
}

async function templatesList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function templatesGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function templatesCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (!name || !subject || !senderEmail || !html) {
    throw new AxiError(
      "templates create requires --name, --subject, --sender-email, --html",
      "VALIDATION_ERROR",
    );
  }
...
async function templatesUpdate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (!body) {
    throw new AxiError(
      "templates update requires --body <json> (the patch object)",
      "VALIDATION_ERROR",
      ['Example: --body \'{"subject":"New subject","htmlContent":"<p>Hi</p>"}\''],
    );
  }
  ...
  return renderResult(
    { updated: { id, fields: Object.keys(body) }, help: ["Run `brevo-axi templates get <id>` to verify"] },
    json,
  );
}

async function templatesTest(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { tested: { template: id, to }, help: ["Check the recipients' inbox for the rendered template"] },
    json,
  );
}

async function templatesDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { deleted: { template: id }, help: ["Run `brevo-axi templates list` to confirm"] },
    json,
  );
}
