...
const TRACK_FLAGS: Record<string, FlagDefinition> = {
  email: { type: "string" },
  contact: { type: "string" },
  name: { type: "string" },
  date: { type: "string" },
...
export async function eventsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown events subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi events --help` to see track, list"],
      );
  }
}

async function eventsTrack(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (props) body["event_properties"] = props;
...
async function eventsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
