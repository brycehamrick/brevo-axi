...
function requireValidEvents(raw: string | undefined, commandPath: string): string[] {
...
export async function webhooksCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown webhooks subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi webhooks --help` to see list, get, create, update, delete"],
      );
  }
}
...
async function webhooksList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const type = oneOf(values, "type", ["transactional", "marketing", "inbound"] as const);
    ...
    webhooks: rows.map((row) =>
      compact({
        ...
        events: Array.isArray(row["events"])
          ? (row["events"] as unknown[]).map(String).join(",")
          : undefined,
...
async function webhooksGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    {
      webhook: compact({
        ...
        events: Array.isArray(payload["events"])
          ? (payload["events"] as unknown[]).map(String).join(",")
          : undefined,
...
async function webhooksCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const type = oneOf(values, "type", ["transactional", "marketing"] as const) ?? "transactional";
  const description = requireString(values, "description", commandPath);
  const events = requireValidEvents(values["events"] as string | undefined, commandPath);
  const json = values["json"] === true;
  requireApiKey(ctx, commandPath);
  ...
  const body: Record<string, unknown> = { url, events, type };
...
async function webhooksUpdate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  const url = requireString(values, "url", commandPath);
  const description = requireString(values, "description", commandPath);
  const rawEvents = values["events"] as string | undefined;
  ...
  return renderResult(
    { updated: compact({ id, fields: Object.keys(body) }), help: [`Run \`brevo-axi webhooks get ${id}\` to verify`] },
    json,
  );
}
...
async function webhooksDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { deleted: { webhook: id }, help: ["Run `brevo-axi webhooks list` to confirm"] },
    json,
  );
}
