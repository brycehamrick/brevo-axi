...
const AUTH_FLAGS: Record<string, FlagDefinition> = { confirm: { type: "boolean" }, json: { type: "boolean" } };

const DELETE_FLAGS: Record<string, FlagDefinition> = { confirm: { type: "boolean" }, json: { type: "boolean" } };

export async function domainsCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown domains subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi domains --help` to see list, get, create, authenticate, delete"],
      );
  }
}

async function domainsList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function domainsGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function domainsCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function domainsAuthenticate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function domainsDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { deleted: { domain: name }, help: ["Run `brevo-axi domains list` to confirm"] },
    json,
  );
}
