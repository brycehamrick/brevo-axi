...
const CONTENT_FLAGS: Record<string, FlagDefinition> = { full: { type: "boolean" }, json: { type: "boolean" } };
...
const SCHEDULED_FLAGS: Record<string, FlagDefinition> = {
  limit: { type: "string" },
  offset: { type: "string" },
  json: { type: "boolean" },
};

const CANCEL_FLAGS: Record<string, FlagDefinition> = { confirm: { type: "boolean" }, json: { type: "boolean" } };

const BLOCKED_FLAGS: Record<string, FlagDefinition> = {
  limit: { type: "string" },
  offset: { type: "string" },
  "start-date": { type: "string" },
  "end-date": { type: "string" },
...
const UNBLOCK_FLAGS: Record<string, FlagDefinition> = { confirm: { type: "boolean" }, json: { type: "boolean" } };
...
export async function emailCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown email subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi email --help` to see send, list, content, events, stats, scheduled, cancel, blocked"],
      );
  }
}

async function emailSend(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (values["param"] !== undefined) {
    const rawParam = values["param"] as string;
    try {
  ...
  let attachment: Array<Record<string, unknown>> | undefined;
  if (attachRaw) {
    try {
  ...
  if (!sandbox && values["confirm"] !== true) {
    throw new AxiError(
      "email send requires --confirm (or --sandbox to validate without sending)",
      "VALIDATION_ERROR",
      [
  ...
  if (senderName || senderEmail) {
    ...
    if (!email_) {
      throw new AxiError(
        "no active sender on this account and none provided",
        "VALIDATION_ERROR",
        [
...
async function emailList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (!email && !messageId && template === undefined) {
    throw new AxiError(
      "email list requires at least one filter: --email, --message-id, or --template",
      "VALIDATION_ERROR",
      ["Brevo refuses unfiltered transactional log queries"],
    );
  }
...
async function emailContent(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function emailEvents(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function emailStats(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  if (daily) {
    ...
    const totals = rows.reduce<{ requests: number; delivered: number; opens: number; clicks: number }>(
      (acc, row) => ({
    ...
    if (rows.length === 0) out["result"] = "0 daily reports in the window";
...
async function emailScheduled(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function emailCancel(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { canceled: { identifier }, help: ["Run `brevo-axi email scheduled <id>` to verify"] },
    json,
  );
}

async function emailBlocked(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function emailUnblock(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { unblocked: { email }, help: ["Run `brevo-axi email blocked` to verify"] },
    json,
  );
}
