...
const NAME_FLAGS: Record<string, FlagDefinition> = {
  name: { type: "string" },
...
export async function foldersCommand(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  const sub = args[0];
  ...
  switch (sub) {
    ...
    default:
      throw new AxiError(
        `unknown folders subcommand: ${sub}`,
        "VALIDATION_ERROR",
        ["Run `brevo-axi folders --help` to see list, get, create, update, delete"],
      );
  }
}

async function foldersList(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function foldersGet(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function foldersCreate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
...
async function foldersUpdate(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  return renderResult(
    { updated: { id, name }, help: ["Run `brevo-axi folders get <id>` to verify"] },
    json,
  );
}

async function foldersDelete(args: string[], ctx: CommandContext): Promise<AxiRenderable> {
  ...
  requireConfirm(
    values,
    commandPath,
    `delete folder ${id} AND every list inside it (irreversible)`,
  );
  await ctx.client.request("DELETE", `/contacts/folders/${encodeURIComponent(id)}`);
  return renderResult(
    { deleted: { id }, help: ["Run `brevo-axi folders list` to confirm the remaining set"] },
    json,
  );
}
